...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(token && API_URL !== '') startApp(); else showLogin();
});

async function api(action,data={}) {
  if(!API_URL) throw new Error('API_URL belum diisi pada script.js.');
  showLoading(true);
  try {
...
async function login(e){
  ...
  try{
    ...
    const j=await apiNoSession('login',{username, password});
